...
pub(crate) fn patch_plt0(plt_data: &mut [u8], plt0_off: usize, plt0_vaddr: u64, got_vaddr: u64) {
    let pc = plt0_vaddr;
    let target_got1 = got_vaddr + 4;
    let target_got2 = got_vaddr + 8;
...
pub(crate) fn generate_helper_code() -> Vec<u8> {
    ...
    let jr = encode_itype(0x67, 0, 5, 0); // jr t0 (jalr x0, t0, 0)
...
pub(crate) fn patch_helper(
    ...
    let off = target_plt_vaddr as i64 - helper_vaddr as i64;
