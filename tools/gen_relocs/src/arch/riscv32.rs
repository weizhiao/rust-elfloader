...
/// Generate PLT (Procedure Linkage Table) for RISC-V 32 architecture
pub(crate) fn generate_plt(
    ...
    // PLT[0]: Special entry for dynamic linker (32-bit version)
    // addi sp, sp, -8
    // sw ra, 4(sp)
    // lw t0, offset(GOT[1])   ; load link_map
    // lw t1, offset+4(GOT[2]) ; load _dl_runtime_resolve
    // jr t1
    text_data.extend_from_slice(&[
        0x13, 0x01, 0x01, 0xff, // addi sp, sp, -8
        0x23, 0x24, 0x11, 0x00, // sw ra, 4(sp)
        0x03, 0x22, 0x00, 0x00, // lw t0, 0(GOT[1]) (will need relocation)
        0x83, 0x22, 0x40, 0x00, // lw t1, 4(GOT[2]) (will need relocation)
    ...
    for func_name in external_funcs {
        ...
        // PLT entry for RISC-V 32-bit:
        // lw t0, offset(GOT)   - load GOT entry address
        // jr t0                - jump to resolved address
        // nop (padding)
        // nop (padding)

        // Calculate GOT entry offset for this function
        let got_entry_offset = got_offset + (got_index * 4);

        // lw instruction uses 12-bit signed immediate
        let got_offset_imm = got_entry_offset as i32 & 0xfff;

        text_data.extend_from_slice(&[
            0x03, 0x22, // lw t0, offset(x0)
        ]);
